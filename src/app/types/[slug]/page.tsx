...
import { getHubArticles, getTestimonials } from "../type-hub-data";
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12 space-y-12">
        ...
        {/* 読者の声（testimonials が空なら非表示） */}
        {testimonials.length > 0 && (
          <section>
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
              <span className="w-1.5 h-6 rounded-full" style={{ background: color }} />
              {t.name.split("（")[0]}タイプと診断された人の声
            </h2>
            <div className="grid gap-3">
              {testimonials.map((tm, i) => (
                <div
                  key={i}
                  className="bg-white border border-line rounded-lg p-5 relative"
                  style={{ borderLeftWidth: "4px", borderLeftColor: color }}
                >
                  <span
                    className="absolute top-3 left-4 text-3xl leading-none opacity-10 select-none"
                    style={{ color }}
                    aria-hidden="true"
                  >
                    &ldquo;
